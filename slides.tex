...
\begin{itemize}
    \item Fronteras de decisión lineales e interpretables
...
\begin{definition}[Error de Predicción Esperado]
...
\]
\end{definition}
\begin{theorem}[Clasificador Óptimo de Bayes]
...
\begin{definition}[Función Discriminante]
...
\begin{definition}[Función Discriminante Transformada]
...
\begin{definition}[Matriz de Dispersión Dentro de Clases]
\[
S_W = \sum_{k=1}^K \sum_{i: y_i = k} (x_i - \mu_k)(x_i - \mu_k)^T = X_c^T X_c = (N - K) \hat{\Sigma}
\]
\end{definition}

\begin{definition}[Matriz de Dispersión Entre Clases]
\[
...
\begin{definition}[Criterio de Fisher]
